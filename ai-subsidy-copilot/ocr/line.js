...
// 本檔不碰資料庫：所有申請資料的讀寫都透過 FastAPI 內部 API（X-Internal-Key），
// 進度／補件／撥款的主動推播則由 FastAPI 端發出（backend/app/services/line_notify.py）。
...
const INTERNAL_KEY = process.env.LINE_INTEGRATION_SECRET || '';
...
async function callApi(method, path, body) {
  ...
  if (!res.ok) throw new Error(`API ${method} ${path} 失敗（${res.status}）`);
  return res.json();
}
...
const CANCEL_TRIGGERS = ['取消申請', '取消', '我要取消', '取消補助申請'];
...
async function handleEvent(event) {
  ...
  if (APPLY_TRIGGERS.includes(text)) {
    const { url } = await callApi('POST', '/api/internal/line/link-code', { line_user_id: userId });
    return client.replyMessage(event.replyToken, {
      type: 'text',
      text:
        `請點選以下連結登入並填寫申請資料、上傳文件（連結 30 分鐘內有效，同時會綁定此 LINE 帳號以接收進度通知）：\n${url}\n\n` +
        `需準備：身分證正反面、購買憑證/發票、存摺封面影本、切結書（如為特定對象或文化語言保存者，另附相關證明）。`,
    });
  }

  if (QUERY_TRIGGERS.includes(text)) {
    const { applications } = await callApi('GET', `/api/internal/line/applications?line_user_id=${encodeURIComponent(userId)}`);
    ...
    const lines = applications.map((r, i) => {
      let entry = `${i + 1}. 受理編號 ${r.public_id}\n   狀態：${r.status_label}`;
      if (r.note) entry += `\n   備註：${r.note}`;
      return entry;
    });
  ...
  if (CANCEL_TRIGGERS.includes(text)) {
    const { cancelled } = await callApi('POST', '/api/internal/line/cancel', { line_user_id: userId });
    return client.replyMessage(event.replyToken, {
      type: 'text',
      text: cancelled
        ? `已為您取消申請案（受理編號：${cancelled}）。若之後想重新申請，隨時輸入「申請」即可。`
        : '目前查無可取消的申請案。',
    });
  }
