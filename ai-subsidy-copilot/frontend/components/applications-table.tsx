...
import { ResultBadge } from "@/components/source-review";
          ...
          <tbody className="divide-y divide-line">
            {applications.map((application) => (
              <tr key={application.public_id} className="group hover:bg-navy-50/50">
                ...
                <td className="px-4 py-4">{application.ai_result ? <ResultBadge result={application.ai_result} /> : <span className="text-slate-400">—</span>}{application.flagged_for_check ? <span className="ml-1 text-xs" title="需進一步查核">🔍</span> : null}</td>
      ...
      <div className="divide-y divide-line lg:hidden">
        {applications.map((application) => (
          <Link href={`/admin/applications/${application.public_id}`} key={application.public_id} className="block p-4 hover:bg-navy-50/50">
            ...
            <p className="mt-3 text-xs font-semibold text-slate-600">{application.product ?? "No documents yet"} · {formatCurrency(application.requested_amount_twd)}</p>
            {application.ai_result ? <div className="mt-2"><ResultBadge result={application.ai_result} /></div> : null}
