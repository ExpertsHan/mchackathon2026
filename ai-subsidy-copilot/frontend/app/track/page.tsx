  ...
  return (
    <PageContainer className="max-w-5xl">
      ...
      {loading ? <LoadingState label="Loading recent applications…" /> : applications.length ? (
        ...
        <Card className="mt-4"><EmptyState icon={<FileSearch className="size-6" />} title={user ? "No applications yet" : "尚未開始申請"} description={user ? "Start a guided application to see it here." : "Sign in with a fictional profile to see recent applications."} action={<Button asChild><Link href={user ? "/apply" : "/login"}>{user ? "Start application" : "Demo login"} <ArrowRight className="size-4" /></Link></Button>} /></Card>
      )}
