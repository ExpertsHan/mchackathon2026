  ...
  if (CANCEL_TRIGGERS.includes(text)) {
    const target = db
      .prepare(`
        ...
        WHERE line_user_id = ? AND status NOT IN ('cancelled', 'rejected', 'disbursed', 'draft', 'approved')
        ORDER BY created_at DESC LIMIT 1
      `)
      .get(userId);
    if (!target) {
      return client.replyMessage(event.replyToken, { type: 'text', text: '目前查無可取消的申請案。' });
    }
    db.prepare(`UPDATE applications SET status = 'cancelled', updated_at = ? WHERE id = ?`).run(new Date().toISOString(), target.id);
    return client.replyMessage(event.replyToken, {
      type: 'text',
      text: `已為您取消申請案（受理編號：${target.id}）。若之後想重新申請，隨時輸入「申請」即可。`,
    });
  }
...
async function pushMessage(userId, text, quickReply) {
  if (!userId) return;
